...
export function SimulationProgress({ onComplete, onCancel, snapshot }: Props) {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 50, background: '#05080f', overflow: 'hidden',
      ...
      <div style={{ position: 'fixed', inset: 0, pointerEvents: 'none', zIndex: 10 }}>
        ...
        {/* 우상단: 배지 */}
        <div style={{ position: 'absolute', top: 30, right: 36, display: 'flex', gap: 10, alignItems: 'center' }}>
        ...
        {/* 하단 중앙: 상태 텍스트 */}
        <div ref={thinkTextRef} style={{ position: 'absolute', bottom: 36, left: '50%', transform: 'translateX(-50%)',
          fontSize: 12, letterSpacing: 3, color: 'rgba(255,255,255,0.2)', whiteSpace: 'nowrap', textTransform: 'uppercase' }}>
          AI 여론 수집 중
        </div>

        {/* 좌하단: 한국 지도 패널 */}
        <div style={{ position: 'absolute', bottom: 26, left: 28, width: 250,
        ...
        {/* 우하단: 실행 제어 */}
        <div style={{ position: 'absolute', bottom: 36, right: 36, pointerEvents: 'auto' }}>
          {onCancel && (
            <button
              onClick={onCancel}
          ...
          <button
            onClick={onComplete}
            style={{ cursor: 'pointer', background: 'transparent',
              border: '1px solid rgba(126,255,200,0.45)', color: '#7effc8',
