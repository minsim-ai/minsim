...
def main() -> None:
    ...
    print(
        f"KoreaSim worker starting queue={QUEUE_NAME} sqlite={store.path} "
        f"interrupted_recovered={len(interrupted)}"
    )
    queue = Queue(QUEUE_NAME, connection=connection)
    worker = SimpleWorker([queue], connection=connection, exception_handlers=[handle_failed_job])
