...
- UI surfaces agent mode/fallback/QA warnings through the result trust layer, but does not expose the full protected `agent_runs` record.

## 7. 2026-07-13 Hardening Addendum

| 영역 | AS-IS | TO-BE / 완료 상태 |
| --- | --- | --- |
| graph | agents 실행 뒤 deterministic scaffold 저장 | Analysis → Report → QA가 실제 compiled LangGraph node로 실행 |
| prior context | agent별 독립 해석 가능 | Report는 Analysis, QA는 Analysis/Report 결과를 안전한 prior output으로 받음 |
| checkpoints | run-level 단일 state 중심 | analysis/report/qa 단계별 KoreaSim SQLite checkpoint 저장 |
| fallback | 완료 결과와 구분 약함 | `mode`, `fallback_reason`, warning, `review_required` 기록 |
| QA | 기록 중심 | fail/warning을 결과 warning에 합치고 grade A downgrade |

- [x] Actual graph output is the source for persisted `orchestration.agents`.
- [x] Each node preserves its prompt version, provider/model, trace, and score metadata.
- [x] Deterministic fallback remains recoverable but requires human review.
- [x] Persona fan-out remains in RQ/async batch and is not converted into graph branches.
