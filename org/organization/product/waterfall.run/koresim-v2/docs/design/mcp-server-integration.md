...
KoreaSim은 MCP로 전환 가능한 상태를 넘어서, 이미 `https://arabesque.cc/mcp`에
MCP 형태의 HTTP endpoint를 운영하고 있다. 기존 구현은 프로젝트·run service를
재사용하고 소유권을 검사하므로 제품 경계는 적절하다. 다음 단계는 simulation
engine을 다시 만드는 작업이 아니라, 현재 custom JSON-RPC transport와 브라우저
session-cookie 인증을 표준 원격 MCP transport와 OAuth resource-server 경계로
교체하는 **production hardening**이다.
