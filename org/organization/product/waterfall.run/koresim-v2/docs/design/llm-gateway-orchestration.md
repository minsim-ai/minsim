...
KoreaSim의 운영 provider는 Upstage `solar-pro2`다. `UPSTAGE_API_KEY`는 Git 밖
로컬 환경에 설치됐고 외부 MCP 10 → 50 → 200 live gate가 통과했다. Gemini는
명시적 rollback compatibility이며 자동 fallback으로 사용하지 않는다.
