...
function buildMetricCards(result: RunResultEnvelope, parseSuccessRate: number | null): V2MetricCard[] {
  return [
    ...
    },
    {
      ...
      detail: result.provider_model ?? result.llm_backend ?? '모델 정보 없음',
    },
  ]
}
...
function buildMethodology(result: RunResultEnvelope, parseSuccessRate: number | null): string[] {
  return [
    ...
    `seed ${result.seed} · 구조화 성공 ${parseSuccessRate === null ? 'N/A' : formatPct(parseSuccessRate)}`,
    `모델 ${result.provider_model ?? result.llm_backend ?? result.model_alias ?? 'unknown'}`,
...
function nameOf(item: RawPersonaResult): string {
  const name = item.persona.name ?? item.persona.persona_name
  return typeof name === 'string' && name.trim() ? name : item.uuid.slice(0, 8)
}
