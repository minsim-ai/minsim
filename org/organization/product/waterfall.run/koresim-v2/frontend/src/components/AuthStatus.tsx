...
export function AuthStatus({ compact = false }: { compact?: boolean }) {
  ...
  if (session.authenticated && session.user) {
    return (
      <button aria-label={compact ? '로그아웃' : undefined} className="ks-auth-button ks-auth-button--secondary" onClick={() => logout('/')} type="button">
  ...
  return (
    <button aria-label={compact ? 'Google 로그인' : undefined} className="ks-auth-button" onClick={() => googleLogin('/app')} type="button">
