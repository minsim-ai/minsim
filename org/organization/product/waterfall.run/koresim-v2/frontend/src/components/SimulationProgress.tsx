...
export function SimulationProgress({
  ...
  const thinkTextRef = useRef<HTMLDivElement>(null);
  ...
  useEffect(() => {
    ...
    scene.fog = new THREE.FogExp2(0x05080f, 0.0006);
    ...
    const starMat = new THREE.ShaderMaterial({
      uniforms: { uTime: { value: 0 }, uColor: { value: new THREE.Color(0xddeeff) } },
    ...
    function drawOutline() {
      ...
      mCtx.closePath();
      mCtx.fillStyle = 'rgba(126,255,200,0.04)'; mCtx.fill();
      mCtx.strokeStyle = 'rgba(126,255,200,0.18)'; mCtx.lineWidth = 1; mCtx.stroke();
      mCtx.beginPath();
    ...
    function drawDots(elapsed: number) {
      ...
      regions.forEach(r => {
        ...
        r.dots.forEach(d => {
          ...
          g.addColorStop(0,  `rgba(126,255,200,${(a * .65).toFixed(3)})`);
          g.addColorStop(.5, `rgba(126,255,200,${(a * .18).toFixed(3)})`);
          g.addColorStop(1,  'rgba(126,255,200,0)');
          ...
          mCtx.fillStyle = `rgba(210,255,230,${(a * .92).toFixed(3)})`; mCtx.fill();
        });
      });
    }
    ...
    function drawPillars(elapsed: number) {
      ...
      regions.forEach(r => {
        ...
        beam.addColorStop(0,   `rgba(126,255,200,${(.92 * shim).toFixed(3)})`);
        beam.addColorStop(.35, `rgba(160,255,210,${(.55 * shim).toFixed(3)})`);
        beam.addColorStop(1,   'rgba(126,255,200,0)');
        mCtx.beginPath();
        ...
        outer.addColorStop(0, `rgba(126,255,200,${(.18 * shim).toFixed(3)})`);
        outer.addColorStop(1, 'rgba(126,255,200,0)');
        mCtx.beginPath();
        ...
        halo.addColorStop(0,  `rgba(126,255,200,${(.9 * shim).toFixed(3)})`);
        halo.addColorStop(.5, `rgba(126,255,200,${(.35 * shim).toFixed(3)})`);
        halo.addColorStop(1,  'rgba(126,255,200,0)');
        mCtx.beginPath(); mCtx.arc(cx, cy, 11, 0, Math.PI * 2); mCtx.fillStyle = halo; mCtx.fill();
        mCtx.beginPath(); mCtx.arc(cx, cy, 2.5, 0, Math.PI * 2);
        mCtx.fillStyle = `rgba(230,255,242,${shim.toFixed(3)})`; mCtx.fill();
      });
    }
    ...
    function mapLoop() {
      ...
      mapRafId = requestAnimationFrame(mapLoop);
    }
    ...
    function tickText() {
      dotN = (dotN + 1) % 4;
      if (thinkTextRef.current) thinkTextRef.current.textContent = PHRASES[pIdx] + '.'.repeat(dotN);
    }
    ...
    const initT = setTimeout(() => {
      startCycle();
      startMap();
      activeIntervals.push(setInterval(tickText, 420));
      activeIntervals.push(setInterval(() => { pIdx = (pIdx + 1) % PHRASES.length; }, 5200));
    }, 500);
  ...
  const content = (
    <div className="ks-sim-progress" style={{ position: 'fixed', inset: 0, zIndex: 50, background: '#05080f', overflow: 'hidden',
      fontFamily: "'Pretendard', -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif", color: '#fff' }}>
      {/* Three.js canvas */}
      ...
      <div style={{ position: 'fixed', inset: 0, pointerEvents: 'none', zIndex: 10 }}>
        ...
        <div className="ks-sim-processed" style={{ position: 'absolute', top: 30, left: 36 }}>
          <div style={{ fontSize: 10, letterSpacing: 4, color: 'rgba(255,255,255,0.28)', textTransform: 'uppercase' }}>PROCESSED</div>
          <span ref={personaNumRef} style={{ display: 'block', fontSize: 32, fontWeight: 700, letterSpacing: -1,
            color: 'rgba(255,255,255,0.75)', lineHeight: 1, marginTop: 6, fontVariantNumeric: 'tabular-nums' }}>{progress.hasSnapshot ? String(progress.done).padStart(3, '0') : '000'}</span>
        </div>

        {/* 우상단: 배지 */}
        <div className="ks-sim-badges" style={{ position: 'absolute', top: 30, right: 36, display: 'flex', gap: 10, alignItems: 'center' }}>
          <div style={{ fontSize: 10, letterSpacing: 2, color: 'rgba(126,255,200,0.85)',
            padding: '5px 13px', border: '1px solid rgba(126,255,200,0.28)', borderRadius: 20, background: 'rgba(126,255,200,0.05)' }}>
            <span style={{ display: 'inline-block', width: 5, height: 5, borderRadius: '50%',
              background: '#7effc8', marginRight: 7, animation: 'simPulse 1.6s ease-in-out infinite' }} />
            인터뷰 진행 중
          </div>
          <div ref={badgeRegionRef} style={{ fontSize: 10, letterSpacing: 2, color: 'rgba(255,255,255,0.28)',
            padding: '5px 13px', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 20 }}>서울</div>
        </div>
        ...
        <div className="ks-sim-stage-copy" style={{ position: 'absolute', top: 86, left: '50%', transform: 'translateX(-50%)',
          ...
          <div style={{ fontSize: 10, letterSpacing: 4, color: 'rgba(126,255,200,0.62)', textTransform: 'uppercase' }}>
            {runLabel}
          </div>
          <h1 style={{ margin: '12px 0 10px', fontSize: 'clamp(30px, 4vw, 54px)', lineHeight: 1.04,
            fontWeight: 700, color: 'rgba(255,255,255,0.9)', letterSpacing: 0 }}>
            {stageTitle}
          </h1>
          <p style={{ margin: '0 auto', maxWidth: 560, fontSize: 14, lineHeight: 1.7, color: 'rgba(255,255,255,0.48)' }}>
            {stageBody}
          </p>
          <div style={{ height: 3, margin: '18px auto 0', maxWidth: 340, borderRadius: 999,
            background: 'rgba(255,255,255,0.08)', overflow: 'hidden' }} aria-label="실행 진행률">
            <span style={{ display: 'block', width: `${Math.max(3, Math.min(100, progressPct))}%`, height: '100%',
              borderRadius: 999, background: 'linear-gradient(90deg, rgba(126,255,200,0.45), rgba(126,255,200,0.95))',
              boxShadow: '0 0 18px rgba(126,255,200,0.36)', transition: 'width .45s ease' }} />
          </div>
        </div>

        {/* 하단 중앙: 상태 텍스트 */}
        <div className="ks-sim-thinking" ref={thinkTextRef} style={{ position: 'absolute', bottom: 36, left: '50%', transform: 'translateX(-50%)',
          fontSize: 12, letterSpacing: 3, color: 'rgba(255,255,255,0.2)', whiteSpace: 'nowrap', textTransform: 'uppercase' }}>
          AI 여론 수집 중
        </div>

        {/* 좌하단: 한국 지도 패널 */}
        <div className="ks-sim-map-panel" style={{ position: 'absolute', bottom: 26, left: 28, width: 250,
          background: 'rgba(4,7,14,0.88)', border: '1px solid rgba(126,255,200,0.16)',
          borderRadius: 12, padding: 14, backdropFilter: 'blur(10px)' }}>
          <div style={{ fontSize: 9, letterSpacing: 3, color: 'rgba(126,255,200,0.45)',
          ...
          <div style={{ marginTop: 10, display: 'flex', alignItems: 'baseline', gap: 6 }}>
            <span ref={counterNumRef} style={{ fontSize: 20, fontWeight: 700,
              color: 'rgba(126,255,200,0.85)', letterSpacing: -0.5, fontVariantNumeric: 'tabular-nums' }}>{progress.done.toLocaleString('ko-KR')}</span>
        ...
        <div className="ks-sim-actions" style={{ position: 'absolute', bottom: 36, right: 36, pointerEvents: 'auto' }}>
          {onCancel && (
            <button
              ...
              style={{ cursor: 'pointer', background: 'transparent',
                border: '1px solid rgba(255,255,255,0.24)', color: 'rgba(255,255,255,0.68)',
          ...
          <button
            ...
            style={{ cursor: canViewResults ? 'pointer' : 'default', background: 'transparent',
              border: `1px solid ${canViewResults ? 'rgba(126,255,200,0.45)' : 'rgba(255,255,255,0.14)'}`,
              color: canViewResults ? '#7effc8' : 'rgba(255,255,255,0.3)',
            ...
            onMouseEnter={e => {
              ...
              el.style.borderColor = 'rgba(126,255,200,0.9)';
              el.style.boxShadow = '0 0 32px rgba(126,255,200,0.18)';
            }}
            onMouseLeave={e => {
              if (!canViewResults) return;
              const el = e.currentTarget;
              el.style.borderColor = 'rgba(126,255,200,0.45)';
