...
**Phase 5/7: Post-demo hardening + Solar live validation**
...
- MCP foundation is implemented at `/mcp` with shared project services, 9 tools, protected-resource metadata, and ownership-aware tests. A dedicated environment-only Bearer API key now provides a single-identity external private-pilot path with constant-time comparison and Origin filtering; it is explicitly separate from `UPSTAGE_API_KEY`. Official SDK transport, multi-user audience-bound OAuth, idempotency, and broader target-client E2E remain tracked in [[docs/execution/mcp-production-hardening-v1]].
