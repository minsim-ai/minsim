"""Run deterministic Campus Policy fixture evaluations."""
...
FIXTURE_PATH = PROJECT_ROOT / "evals" / "fixtures" / "campus_policy_fixture.json"
...
def run_fixture(fixture: dict[str, Any]) -> dict[str, Any]:
    total_responses, parse_failed, metrics = build_campus_policy_result(fixture)
...
def build_campus_policy_result(fixture: dict[str, Any]) -> tuple[int, int, dict[str, Any]]:
    from src.agent.simulator import SimResult
    from src.simulations.campus_policy import aggregate_campus_policy, make_campus_policy_parser
    ...
    parser = make_campus_policy_parser(input_data)
    parsed_results = [
    ...
    metrics = aggregate_campus_policy(input_data, raw_results, parsed_results)
